...
fasterGetSimulationScheme <- function(lab, condMeanX, condMeanX2, Xq.value) {
  ...
  for (ii in 1:nrow(triangles)) {
    ...
    if (any(is.in.chull)) {     
      ...
      trg[, concerned] <- idx
      Ps <- cbind(condMeanX[concerned], condMeanX2[concerned])
      X <- cbind(Xq[idx], Xq[idx]^2)
      P <- Ps
      class(P) <- "numeric"      
      P <- matrix(P , nrow = nrow(Ps), ncol= ncol(Ps))
      probs[concerned, ] <- cart2bary(X, P)
    }    
  }
  if (any(is.na(trg[1, ]))) {
    #browser()   ### commented out by cabral....
  }
...
#<Cabral>
simulateParsimoniouslyXgivenW <- function(W, xmin, xmax, Xq, condMeanX, sigma2, parameters, r=3, condMeanX2=NULL) {
  #<\Cabral>
  ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ## Validate arguments
  ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  #<Cabral>
  if(!is.null(condMeanX2)){
    if(length(W) != length(condMeanX2)){
      throw("Vecotrs 'W' and 'condMeanX2 must be of same length.")
    }
  }
  #<\Cabral>
  ...
  ##
  ## removing NA from the simulationSchemes matrix..... might be dangereous..
  ##
  if(any(is.na(simulationSchemes))){    
    # simulationSchemes <- simulationSchemes[ - which(is.na(simulationSchemes[,1]))   ,  ]
    simulationSchemes[ which(is.na(simulationSchemes[,1]))   ,  ] <- simulationSchemes[ 1  ,  ]    
  }
