  ...
  if (!cleverCovTheta) {
    ...
    if (!exact) { ## do not use clever covariate nor exact expression
      dev <- function(XW) {
        ...
        W <- XW[, 2, drop=FALSE];
        if(isBetaMult){
          ( devThetaAux(XW) * ( derivF_beta(X,fW(XW)) - mu(W)/g(W)*(X==0)) ) %*% invSigma   
        }
        else{
      ...
      theta1 <- function(XW) {
        ...
        if( isBetaMult){
          term1 <- ( devThetaAux(XW) *  ( derivF_beta(X,fW(XW))  -   mu(W)/g(W)*(X==0))) %*% invSigma;        
          term2 <- ( derivF_beta(X,fW(W)) %*% (TXW - theta(cbind(X=0, W=W)) - f_beta(X,fW(W))) %*% invSigma );          
...
setMethodS3("updateThetaTab", "NPVI", function(this, dev, cleverCovTheta, exact=TRUE, ...) {
  ...
  if (!cleverCovTheta) {
    ...
    if(isBetaMult){      
      d <- ncol(muW)
      nbRow <- length(c(rep(muW[,1], each=length(Xq.value)), muW[,1]))
      muWBis <- matrix(NA, ncol = d, nrow = nbRow)
      for(i in 1:d){
        muWBis[,i] <- c(rep(muW[,i], each=length(Xq.value)), muW[,i])
      }
      muW <- muWBis
      rm(muWBis)
    }
    ...
    if(isBetaMult){
      Xfull_OBSTAB <- X
      Wfull_OBSTAB <- extractW(OBSTAB)  
      
      derivF_betaXW <- derivF_beta(Xfull_OBSTAB, Wfull_OBSTAB) 
      f_betaXW <- f_beta(Xfull_OBSTAB, Wfull_OBSTAB)      
      
      thetaXW_full <- thetaXW
      theta0W_full <- theta0W      
      invSigma <- solve(sigma2)       
    }
    rm(g, mu)
    #<\Cabral>
    ...
    if(isBetaMult){      
      devXW <- (as.vector(devThetaAuxXW) * ( derivF_betaXW  -   muW/gW*(Xfull_OBSTAB==0))) %*% invSigma
    }
    else{
      devXW <- devThetaAuxXW * (X - muW/gW*(X==0))/sigma2
    }
    ...
    if (!exact) { ## do not use clever covariate nor exact expression
      if(isBetaMult){
        epsTerm1 <-  devXW %*% eps
        #epsTerm1_Mat <- matrix( epsTerm1 , nrow=nr, ncol=nr)
        theta1XW <- thetaXW + epsTerm1 
      }
      else{
      ...
      if(isBetaMult){
        ...
        term2 <- derivF_betaXW * (thetaXW_full - theta0W_full - f_betaXW);
        term2 <- term2 %*% invSigma
        epsTerm2 <-  ( term2 * thetaXW_full ) %*% eps   
        #epsTerm2_Mat <- matrix( epsTerm2 , nrow=nr , ncol=nr)
        
        epsTerm2_Den <- term2 %*% eps
        #epsTerm2_Den_Mat <- matrix( epsTerm2_Den , nrow = nr , ncol = nr)
        
        numerator <- thetaXW + ( epsTerm1 + epsTerm2 )
        denominator <- 1 + epsTerm2_Den
      }
      else{
        theta0W <- theta0(1:nr)
