...
###
### use to determien whether the value of the 
###

    ff = NULL,
### A formula used in the estimated version to compute the value of the parameter
### of interest.
###

    d = 1,
### the number of covariates contained in the estimation.
### We assume here that the user will have a clear enough view on the formula in
### order to avoid any identifiable problem with the formula.
    covariatesMean = rep(0, d),
###
### mean of the additionnal covariatest that will be produce 
###

    covariatesSigma = diag(1, d),
###
### covariance of the additionnal covariate that will be produce
###
...
### \code{FALSE}.
      ...
      if(isBetaMult){
        ## creating the vector of observation
        theX0 <- O[2,"X"];
        obsFF <- cbind(X,Y,W)
        obsFF[, "X"] <- obsFF[, "X"] - theX0;
        
        ## Adding covariates to the observations      
        covariates <- mvrnorm(n, mu=covariatesMean, Sigma = covariatesSigma)
        colnames(covariates) <-  paste("V", 1:d, sep="") 
        
        correlation <- covariatesSigma
        mean <- covariatesMean    
        
        obsFF <- cbind( obsFF[,c("Y","X")], covariates, W = obsFF[,"W"])      
        
        ## Deriving the corresponding functions 
        internalFunctions <- create_fBeta_And_DerivFBeta(ff=ff,obs=obsFF)
        ...
        mode <- mode(f_beta)
        if(mode != "function"){
          throw("the parameter 'f_beta' should a function and not a " , mode)
        }
        
        mode <- mode(derivF_beta)
        if(mode != "function"){
          throw("the parameter 'derivF_beta' should a function and not a " , mode)
        }
      }
      ...
      if(isBetaMult){
        ## computing the matrix Sigma       
        Xin <- obsFF[,"X"]
        Win <- extractW(obsFF)
        
        ## Adding dummies variables to Win
        derivFBeta <- derivF_beta(Xin, Win)
        
        SimSigma.list <- lapply(1:nrow(derivFBeta), function(ii) {
          row <- matrix(as.numeric(derivFBeta[ii, ]), ncol=1)
          mat <- row %*% t(row)
        })
        
        Sigma <- Reduce("+", SimSigma.list)/nrow(derivFBeta)      
      ...
      if(isBetaMult){
        ## constructing the indice based observations matrix      
        res <- estimatePsi(theta=theta, theta0=theta0 , fX=fX, sigma2=Sigma, isBetaMult=TRUE, derivF_beta = derivF_beta, derivH_beta=derivH_beta, obs=obsFF, realObs=obsFF , isSimulationScheme = TRUE, verbose =verbose)        
        #estimatePsi <- function(theta, theta0, fX, obs, sigma2, realObs=NULL, isBetaMult=FALSE, derivF_beta = NULL, isSimulationScheme=FALSE, ..., verbose=FALSE)
        ...
        effIC <- function(obs) {
          ...
          eic1.interm <- derivF_beta(X,Wfull)*(theta( cbind( X = obs[, "X"] , W )) - theta0(W) - f_beta(X,Wfull, beta = truePsi))
          
          eic1 <- sapply( 1:nrow(eic1.interm) , function(ii){
            row <- invSigma %*% t(eic1.interm[ii,,drop=FALSE])  
          })        
          
          #extracting relevant values of W needed for the computation of muW and gW     
          
          selectedCol <- names(truePsi)
          posIntercept <- match("intercept", selectedCol)
          if(!is.na(posIntercept)){
            posIntercept <- -1*posIntercept
            selectedCol <- selectedCol[posIntercept]
          }
          
          #relevantW <- Wfull[,selectedCol]
          relevantW <- Wfull
          eic2.interm <- (Y - theta(cbind( X = obs[, "X"] , W ) )) * (derivF_beta(X,Wfull) - mu(relevantW[,"W"])/g(relevantW[,"W"])*(X==0))
          
          eic2 <- sapply(1:nrow(eic2.interm) , function(ii){
            row <- invSigma %*% t(eic2.interm[ii,,drop=FALSE])    
          })
