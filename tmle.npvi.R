...
### definition of the "effect" of \eqn{X} on \eqn{Y}.}}
    ...
    ff=NULL, 
### A formula used to define the number of covariates to create for our 
### estimation, considering here a parameter of interest which is 
### multidimensional (d>1). If the paraemter is left empty, it will be defaulted to the 
### exact number of covariates as defined in the parameter "obs".

    isBetaMult=FALSE,
### enable the computation of the paremeter of interest, called Beta, as if
### it is a multidimensional (d>1)
...
### for flavor \code{learning}.
      ...
      ### <Cabral>
      ## introducing variables which will be most needed in the case of a multidimensional parameter of interest
      d <- 1
      #ObsXW <- obs[,"X"] * extractW(obs)
      sigmamin=-Inf
      sigmamax=Inf
      ...
      if(isBetaMult){
        if(missing(ff)){
          W <- extractW(obs)
          ff <- as.formula(paste("Y~" , paste("I(" , colnames(W), ")", collapse="+"), sep=""))
        }
        
        ## Calculating variables only necessary for the
        derivH <- identifiablePseudoObs(ff, obs)
        #derivF <- obs[,"X"] * derivH        
        
        d <- ncol(derivH)
        #mumin <- apply(derivF , 2, min)
        #mumax <- apply(derivF , 2 , max)
        
        # introducting min and max of sigma which is defined as sigma = E[ X^2 | W]
        sigmamin <- min(obs[,"X"]^2)
        sigmamax <- max(obs[,"X"]^2)
      }
      ### <\Cabral>
...
tmle.npvi <- function(obs,    ff=NULL, isBetaMult=FALSE,     f=identity,        nMax=30L,
                      flavor=c("learning", "superLearning"),  lib=list(),  nodes=1L,  cvControl=NULL,
                      family=c("parsimonious",   "gaussian"),  
                      cleverCovTheta=FALSE, bound=1, B=1e5, trueGMu=NULL, trueTheta=NULL, iter=5L,
  ...
  flavor <- match.arg(flavor)
  tmle <- try(tmle.npvi.(obs=obs, ff=ff, isBetaMult=isBetaMult, f=f, nMax=nMax, flavor=flavor, lib=lib, nodes=nodes, cvControl=cvControl,
                         family=family, cleverCovTheta=cleverCovTheta, bound=bound, B=B,
                         trueGMu=trueGMu, iter=iter, trueTheta=trueTheta,
  ...
  if (flavor=="superLearning" & failed) {    
    tmle <- tmle.npvi.(obs=obs, ff=ff, isBetaMult=isBetaMult, f=f, nMax=nMax, flavor="learning", lib=tmle.npvi::learningLib, nodes=1L,
                       cvControl=cvControl, family=family, cleverCovTheta=cleverCovTheta, bound=bound, B=B, 
                       trueGMu=trueGMu, trueTheta,trueTheta, iter=iter, stoppingCriteria=stoppingCriteria, gmin=gmin, gmax=gmax,
                         mumin=mumin, mumax=mumax, verbose=verbose, tabulate=tabulate, exact=exact, light=light)
