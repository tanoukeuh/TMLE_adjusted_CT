#<Cabral>
simulateData <- function(B, W, X, Xq, g, mu, muAux, sigma2, theta=NULL, Y=list(value=NA, index=NA), weightsW=rep(1, length(W)), family=c("parsimonious", "gaussian"), verbose=FALSE, sigmaAux = NULL) {
#<\Cabral>
  ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ## Validate arguments
  ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if(!is.null(sigmaAux)){
    ...
    if( mode(sigmaAux) != 'function'){
      throw("The parameter 'sigmaAux' should be a function and not ", mode(sigmaAux))
    }
  }
  #<\Cabral>
  ...
  if (family=="gaussian") {
    ...
    indices <- simulateParsimoniouslyXgivenW(WB[U], min(obsX), max(obsX),
                                               Xq, condMeanX, sigma2, parameters,condMeanX2=condMeanX2)
