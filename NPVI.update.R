...
setMethodS3("update", "NPVI", function(object,
                                       ...
                                       bound=1e-1, B=1e4,
                                       light=TRUE, 
                                       cleverCovTheta=TRUE, useTrueTheta = FALSE,
                                       exact=TRUE, trueGMu=NULL, trueTheta = NULL,
  ...
  if (cleverCovTheta) {
    ...
    obsB <- simulateData(B, obs[, "W"], obsT[, "X"], Xq, g, mu, muAux, sigma2,
                         theta=theta, Y=Yq, weightsW=weightsW, family=family, sigmaAux=sigmaAux) 
    ...
    #<Cabral>
    if(isBetaMult){
      invSigma <- solve(sigma2)
      Wfull <- fW(obsB)
      fBetaXW <- f_beta(X,Wfull)
      derivFBetaXW <- derivF_beta(X,Wfull)
      
      D1_intermediary <- (thetaXW - theta0W - fBetaXW)*derivFBetaXW ;
      D1 <- sapply( 1:nrow(D1_intermediary) , function(ii){
        row <- matrix(as.numeric(D1_intermediary[ii,] %*% invSigma) , ncol = ncol(D1_intermediary))    
      })
      
      D2_intermediary <- (Y - thetaXW) * (derivFBetaXW - muW/gW*(X==0))
      ...
      partialDiv <- apply(abs(eic), 2 , mean)
      
      if( sum(is.na(partialDiv)) != 0 ){
        #browser()
      }
      rm(eic1, eic2, eic , D1_intermediary, D2_intermediary, D1 , D2, derivFBetaXW , Wfull, fBetaXW );      
    }
    else{         
  ...
  if (!useTrueGMu) {
    ...
    if(isBetaMult){
      ...
      effICW <- function(W) {
        ...
        if(isBetaMult){
          h_betaW <- h_beta(realW)
          derivH_betaW <- derivH_beta(realW)
          invSigma <- solve(sigma2)
          #W <- as.matrix(W)
          out <- (  derivH_betaW*as.vector(condExpXYgivenW(realW)) - theta0(W)*mu(W) - (h_betaW * derivH_betaW)* as.vector(condExpX2givenW(realW) ) ) %*% invSigma;  
        }
        else{
    ...
    #<Cabral>
    gW <- g(extractW(obs))
    devG <- estimateDevG(gW, obsT, eic1, flavor=flavor, learnDevG=learnDevG, light=light, isBetaMult = isBetaMult,
                         SuperLearner.=SuperLearner.,
                         ..., verbose=verbose);    
    ...
    #<Cabral>
    if(isBetaMult){
      puremu <- getPureMu(this)
      muW <- puremu(extractW(obs))
    }
    else{
      muW <- mu(extractW(obs))
    }  
    ...
    devMu <- estimateDevMu(muW, obsT, eic1, flavor=flavor, learnDevMu=learnDevMu, light=light, isBetaMult=isBetaMult, 
                           SuperLearner.=SuperLearner.,
                           ..., verbose=verbose); 
    ...
    ## Update 'sigma2'    
    if(isBetaMult){
      sigmaW <- sigma(extractW(obs))
      devSigma <- estimateDevSigma(sigmaW, obsT, eic1, flavor=flavor, learnDevMu=learnDevMu, light=light, 
                             SuperLearner.=SuperLearner.,
                             ..., verbose=verbose);    
      updateSigma(this, devSigma, exact=exact, effICW=effICW);  
    }
    else{
      X <- fX(obs)
